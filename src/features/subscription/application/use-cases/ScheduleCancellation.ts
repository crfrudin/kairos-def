...
import { SubscriptionDate } from '../../domain';
...
export class ScheduleCancellation {
  ...
  async execute(input: {
    ...
    readonly cancelEffectiveOn: string; // YYYY-MM-DD (sem Date/now)
  }): Promise<ResultType<SubscriptionSuccess, SubscriptionFailure>> {
    try {
      const sub = await this.repo.getOrCreateForUser(input.userId);

      const date = SubscriptionDate.create(input.cancelEffectiveOn);

      const events = sub.scheduleCancellation(date);
